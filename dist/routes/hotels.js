...
router.post("/:hotelId/bookings/payment-intent", auth_1.default, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        const paymentIntent = yield razorpay.orders.create(options);
        if (!paymentIntent) {
            return res.status(500).json({ message: "Error creating payment intent" });
        }
        const response = {
            paymentIntentId: paymentIntent.id,
            clientSecret: process.env.RAZORPAY_KEY_ID, // Correct usage here
            totalCost,
        };
        res.send(response);
    }
...
router.post("/:hotelId/bookings", auth_1.default, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const paymentIntent = yield razorpay.orders.fetch(paymentIntentId);
        if (!paymentIntent) {
            return res.status(400).json({ message: "Payment intent not found" });
        }
        const notes = paymentIntent.notes || {};
        if (notes.hotelId !== req.params.hotelId ||
